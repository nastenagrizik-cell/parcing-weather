...
RP5_CANONICAL_COLUMNS = [
    ...
    'T','Po','P','Pa','U','DD','Ff','ff10','ff3','N','WW','W1','W2','Tn','Tx','Cl','Nh','H','Cm','Ch','VV','Td','RRR','tR','E','Tg','E_prime','sss'
]
COLUMN_ALIASES = {'E\'': 'E_prime', 'E_prime': 'E_prime'}
...
def extract_station_name(city_name: str, rp5_url: str) -> str:
...
def _normalize_weather_rp5_frame(df: pd.DataFrame, city_name: str, rp5_url: str, wmo_id: str | None) -> pd.DataFrame:
    frame = df.copy()
    ...
    frame['station_name'] = extract_station_name(city_name, rp5_url)
    frame['wmo_id'] = wmo_id
...
def fetch_city_weather(city_name: str, rp5_url: str, date_from: str, date_to: str) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        raise ValueError(f'Не удалось получить данные по станции {city_name}')
    return _normalize_weather_rp5_frame(df, city_name, rp5_url, str(station_id)).reset_index(drop=True)
