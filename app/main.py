    ...
    try:
        ...
        for city in cities:
            try:
                ...
                messages.append(f"OK: {city.name} -> {station['station_name']} -> {sheet_name} ({len(frame)} rows)")
...
@app.post('/cities/add')
def add_city(name: str = Form(...), is_active: str | None = Form(None), db: Session = Depends(get_db)):
    clean_name = name.strip()
    existing = db.query(City).filter(City.name == clean_name).first()
    if existing:
        return RedirectResponse('/', status_code=303)
    station = resolve_station(clean_name)
    city = City(name=clean_name, rp5_url=station['url'], sheet_name=sanitize_sheet_name(clean_name), is_active=bool(is_active))
    db.add(city)
    db.commit()
    return RedirectResponse('/', status_code=303)
...
def bulk_add_cities(cities_text: str = Form(...), db: Session = Depends(get_db)):
    ...
    added = 0
    for line in lines:
        existing = db.query(City).filter(City.name == line).first()
        if existing:
            continue
        try:
            station = resolve_station(line)
            city = City(name=line, rp5_url=station['url'], sheet_name=sanitize_sheet_name(line), is_active=True)
            db.add(city)
            added += 1
        except Exception:
            logger.exception('FAILED bulk add city: %s', line)
    if added:
        db.commit()
    return RedirectResponse('/', status_code=303)
