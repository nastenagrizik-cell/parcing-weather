...
app = FastAPI(title='RP5 Free Export')
...
async def export_weather(
    ...
    required = {'name', 'rp5_url', 'sheet_name'}
    if not reader.fieldnames or not required.issubset(set(reader.fieldnames)):
        raise HTTPException(status_code=400, detail='CSV должен содержать колонки: name, rp5_url, sheet_name')

    city_frames = {}
    processed = 0
    for row in reader:
        name = (row.get('name') or '').strip()
        rp5_url = (row.get('rp5_url') or '').strip()
        sheet_name = normalize_sheet_name((row.get('sheet_name') or name).strip())
        if not name or not rp5_url:
            continue
        city_frames[sheet_name] = fetch_city_weather(name, rp5_url, date_from, date_to)
        processed += 1
